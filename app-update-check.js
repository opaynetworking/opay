/* app-update-check.js — AGGRESSIVE DETECTION + FULL FEATURES
 * ----------------------------------------------------------------------------
 ...
 *
 * Features:
 *   • Fast display (no long delays, cached version.json)
 *   • Modal 1: Update Required + Update Now + WhatsApp Channel
 *   • Modal 2: Install instructions + Continue to Download + WhatsApp + Maybe Later
 *   • Handles the "Blocked / Install unknown apps" scenario
 *   • Snooze for 3 hours
 * ==========================================================================*/
...
(function () {
    ...
    var OPTS = Object.assign({
        ...
        cacheTtlMs: 5 * 60 * 1000           // cache version.json for 5 min
    }, window.APP_UPDATE_OPTIONS || {});

    var SNOOZE_KEY = 'opayUpdateSnoozedUntil';
    var VERSION_CACHE_KEY = 'opayVersionJsonCache';
    ...
    function readCachedVersion() {
        try {
            ...
            if (Date.now() - cached.timestamp > OPTS.cacheTtlMs) return null;
            return cached.data;
        } catch (e) { return null; }
    }

    function writeCachedVersion(data) {
        try {
    ...
    function injectStyles() {
        ...
        style.textContent =
            '.app-update-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.78);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;z-index:2147483000;padding:20px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;opacity:0;transition:opacity 0.2s ease;-webkit-tap-highlight-color:transparent;overflow-y:auto;}' +
            '.app-update-overlay.show{opacity:1;}' +
            '.app-update-card{background:' + cardBg + ';color:' + cardText + ';width:90%;max-width:380px;border-radius:28px;padding:30px 24px 22px;text-align:center;box-shadow:0 25px 60px -12px rgba(0,0,0,0.5);transform:scale(0.92) translateY(12px);transition:transform 0.3s cubic-bezier(0.21,1.11,0.38,1);margin:auto;}' +
    ...
    function showUpdateModal(info, installedVer, installedName) {
        ...
        if (nowBtn) {
            nowBtn.addEventListener('click', function () {
                ...
                // Old APK — show install instructions first
    ...
    /* ═══════════════════════════════════════════════════════════════
     *  MODAL 2 — INSTALL INSTRUCTIONS
     *  Shows the steps + Continue to Download + WhatsApp + Maybe Later
     * ═══════════════════════════════════════════════════════════════ */
        ...
        instructions.innerHTML =
            '<div class="app-update-card" role="dialog" aria-modal="true">' +
                ...
                '<div class="app-update-steps">' +
                    '<div class="step"><span class="num">1.</span><span>Tap <b>Continue to Download</b> below.</span></div>' +
                    '<div class="step"><span class="num">2.</span><span>When Android says <b>"Blocked"</b> or <b>"Install unknown apps"</b>, tap <b>Settings</b>.</span></div>' +
                    '<div class="step"><span class="num">3.</span><span>Turn ON <b>"Allow from this source"</b>.</span></div>' +
                    '<div class="step"><span class="num">4.</span><span>Go back and tap <b>Install</b> again.</span></div>' +
                '</div>' +
                '<div class="app-update-buttons">' +
     ...
     * ═══════════════════════════════════════════════════════════════ */
    function processVersionData(info) {
        if (!info || !info.apkUrl) {
        ...
        console.log('[app-update] Latest versionCode:', latest, '| Installed v' + installed);
    ...
    function fetchVersionJson() {
        var url = OPTS.versionJsonUrl + '?t=' + Date.now();
        return fetch(url, { cache: 'no-store' })
            ...
            .catch(function (err) {
                ...
                if (cached) {
                    console.log('[app-update] Using cached version.json');
                    return cached;
                }
                return null;
            });
    ...
    function checkForUpdate() {
        ...
        console.log('[app-update] Running check...');

        // FAST PATH — use cached data if fresh
        var cached = readCachedVersion();
        if (cached) {
            console.log('[app-update] Using cached data — showing instantly.');
            processVersionData(cached);
            fetchVersionJson(); // refresh in background
            return;
        }

        // SLOW PATH — fetch and process
        fetchVersionJson().then(function (info) {
            if (info) processVersionData(info);
        });
    }
    ...
    console.log('[app-update] Loaded (aggressive detection + full features).');
})();
